...
# Page configuration
st.set_page_config(
    page_title="MADv1.5 (r2SCAN) Explorer",
    page_icon="🔬",
    layout="wide",
...
# --- Constants ---
DATASET_FILES = {
    "mad-1.5-r2scan-train": "./pages/MADv1.5_Dataset/mad-1.5-r2scan-train.xyz",
    "mad-1.5-r2scan-test": "./pages/MADv1.5_Dataset/mad-1.5-r2scan-test.xyz",
    "mad-1.5-r2scan-val": "./pages/MADv1.5_Dataset/mad-1.5-r2scan-val.xyz",
}
CACHE_DIR = Path("./pages/madv1.5_explorer_cache")
DIMENSIONALITY_LABELS = {0: "0D (Molecule/Cluster)", 1: "1D (Wire/Chain)", 2: "2D (Slab/Surface)", 3: "3D (Bulk)"}
MAD_PAPER_URL = "https://doi.org/10.48550/arXiv.2603.02089"
MAD_ARCHIVE_URL = "https://archive.materialscloud.org/record/2026.50"
MAD_DOWNLOAD_URLS = {
    "Training split": "https://archive.materialscloud.org/records/18tke-tt476/files/mad-1.5-r2scan-train.xyz?download=1",
    "Validation split": "https://archive.materialscloud.org/records/18tke-tt476/files/mad-1.5-r2scan-val.xyz?download=1",
    "Test split": "https://archive.materialscloud.org/records/18tke-tt476/files/mad-1.5-r2scan-test.xyz?download=1",
}

# --- Session state defaults ---
for key, default in [
    ('madv15_current_dataset', None),
    ('madv15_query_results', []),
    ('madv15_selected_indices', []),
    ('madv15_dataset_loaded', False),
    ('madv15_all_elements', []),
    ('madv15_stats', None),
    ('madv15_frames', []),
    ('madv15_dim_indices', None),
    ('madv15_viz_pos', 0),
    ('madv15_query_elapsed_seconds', None),
]:
...
def load_trajectory1(dataset_name: str):
...
def render_dataset_citation():
    """Show the MAD-1.5 citation and official Materials Cloud downloads."""
    with st.expander("Dataset citation and official downloads"):
        st.markdown(
            "**Please cite:** C. Malosso, F. Bigi, P. Pegolo, J. W. Abbott, "
            "P. Loche, M. Rossi, M. Ceriotti, and A. Mazitov, “High-quality, "
            "high-information datasets for universal atomistic machine learning,” "
            f"*arXiv:2603.02089* (2026). [Preprint]({MAD_PAPER_URL})"
        )
        st.markdown(f"**Official dataset record:** [Materials Cloud]({MAD_ARCHIVE_URL})")
        st.markdown(
            "**Direct r2SCAN extXYZ downloads:** "
            + " · ".join(
...
def main():
    st.title("🔬 MADv1.5 (r2SCAN) Explorer")
    st.caption("Extended XYZ Trajectory Dataset Explorer for `mad-1.5-r2scan-train.xyz`, `mad-1.5-r2scan-test.xyz`, `mad-1.5-r2scan-val.xyz`")
    ...
    with st.sidebar:
        ...
        if not available:
            st.error("No dataset files found! Place `mad-1.5-r2scan-train.xyz`, `mad-1.5-r2scan-test.xyz`, `mad-1.5-r2scan-val.xyz` in the working directory.")
            return
        ...
        if st.button("Load Dataset", type="primary"):
            with st.spinner(f"Loading {available[dataset_name]}..."):
                frames = load_trajectory1(dataset_name)
                if frames is None or len(frames) == 0:
                    st.error("Failed to load or empty dataset.")
                else:
                    st.session_state.madv15_frames = frames
                    st.session_state.madv15_current_dataset = dataset_name
                    st.session_state.madv15_dataset_loaded = True
                    st.session_state.madv15_query_results = []
                    st.session_state.madv15_selected_indices = []
                    st.session_state.madv15_query_elapsed_seconds = None

                    with st.spinner("Computing statistics (cached on disk)..."):
                        stats = compute_statistics(dataset_name, frames)
                        st.session_state.madv15_stats = stats
                        st.session_state.madv15_all_elements = stats['elements']

                    with st.spinner("Building dimensionality index (cached on disk)..."):
                        dim_indices = get_or_build_dim_subsets(dataset_name, frames)
                        st.session_state.madv15_dim_indices = dim_indices
        ...
        if st.session_state.madv15_dataset_loaded:
            st.success("✅ Dataset loaded")
            st.info(
                f"**File:** "
                f"{DATASET_FILES[st.session_state.madv15_current_dataset]}"
            )
            s = st.session_state.madv15_stats
    ...
    # --- Main content ---
    if not st.session_state.madv15_dataset_loaded:
    ...
    frames = st.session_state.madv15_frames
    stats = st.session_state.madv15_stats
    dim_indices = st.session_state.madv15_dim_indices
    ...
    with tab1:
        ...
        if st.button("🔍 Search", type="primary", use_container_width=True):
            ...
            with st.spinner("Searching..."):
                ...
                st.session_state.madv15_query_results = results
                st.session_state.madv15_query_elapsed_seconds = search_elapsed
                st.session_state.madv15_selected_indices = []
                st.session_state.madv15_viz_pos = 0
            st.success(
    ...
    with tab2:
        ...
        query_elapsed = st.session_state.madv15_query_elapsed_seconds
        ...
        if not st.session_state.madv15_query_results:
            st.info("No results yet. Run a query in the Query tab.")
        else:
            results = st.session_state.madv15_query_results
            ...
            with c1:
                if st.button("Select All"):
                    st.session_state.madv15_selected_indices = list(range(len(results)))
                    st.rerun()
            with c2:
            ...
            if select_range:
                ...
                st.session_state.madv15_selected_indices = [
                    i for i in selected if 0 <= i < len(results)
                ]

            selected_manual = st.multiselect(
                "Or select manually:",
                options=list(range(len(results))),
                default=st.session_state.madv15_selected_indices,
                format_func=lambda x: f"{x}: {results[x]['formula']} ({results[x]['natoms']} atoms)"
            )
            st.session_state.madv15_selected_indices = selected_manual

            # Export
            if st.session_state.madv15_selected_indices:
                st.success(
                    f"{len(st.session_state.madv15_selected_indices)} "
                    "structure(s) selected"
                )
                if st.button("📥 Export Selected as Extended XYZ", type="primary"):
                    content = export_to_extxyz(
                        results, st.session_state.madv15_selected_indices
                    )
                    st.download_button(
            ...
            if st.session_state.madv15_selected_indices:
                ...
                sel = st.session_state.madv15_selected_indices
                n_sel = len(sel)

                # Clamp position
                if st.session_state.madv15_viz_pos >= n_sel:
                    st.session_state.madv15_viz_pos = n_sel - 1
                if st.session_state.madv15_viz_pos < 0:
                    st.session_state.madv15_viz_pos = 0
                ...
                with nav1:
                    if st.button("⏮ First", use_container_width=True):
                        ...
                        st.rerun()
                with nav2:
                    if st.button("◀ Prev", use_container_width=True, disabled=(st.session_state.madv15_viz_pos == 0)):
                        st.session_state.madv15_viz_pos -= 1
                        st.rerun()
                with nav3:
                    st.markdown(
                        f"<div style='text-align:center; padding:0.5rem; font-size:1.1rem;'>"
                        f"<b>{st.session_state.madv15_viz_pos + 1}</b> / {n_sel}</div>",
                        unsafe_allow_html=True
                    )
                with nav4:
                    if st.button("Next ▶", use_container_width=True, disabled=(st.session_state.madv15_viz_pos >= n_sel - 1)):
                        st.session_state.madv15_viz_pos += 1
                        st.rerun()
                with nav5:
                ...
                viz_index = sel[st.session_state.madv15_viz_pos]
                r = results[viz_index]
